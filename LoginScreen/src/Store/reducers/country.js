...
export const uploadImageFile = async (formData) => {
  ...
  let res = fetchClient
    .post(`${constants.API.COUNTRY.FILE_UPLOAD}`, formData, config)
    .then((res) => res.data)
    .catch((error) => error);
...
export const callGetSnapShotAPI = createAsyncThunk(
  ...
  async (payload = {}) => {
    let res = await fetchClient
      .post(`${constants.API.HOME.GET_POPULAR_COUNTRIES}?listcount=${payload.snapshot_count}`,
        {
          solutionName: ["Employee Management - Labor and Employment"],
          CountryIds: [payload.id],
        }
...
export const callGetSpecificsAPI = createAsyncThunk(
  ...
  async (payload = {}) => {
    let res = await fetchClient
      .post(`${constants.API.HOME.GET_POPULAR_COUNTRIES}?listcount=${payload.specifics_count}`,
        {
...
export const callGetSpecificsDataforLifecycleAPI = createAsyncThunk(
  ...
  async (payload = {}) => {
    let res = await fetchClient
      .post(`${constants.API.DETAILS.GET_SPECIFICS_DATA}${payload._sTopicContentId ? '/' + payload._sTopicContentId : ''}`,
        [
          ...payload.specificsId,
        ]
      )
...
export const getAllCountriesList = createAsyncThunk(
  ...
  async (payload = {}) => {
    let res = await fetchClient
      .post(`${constants.API.COUNTRY.GET_ALL_COUNTRY_INFO_LIST}`, { ...payload.body })
      .then(res => {
        return { ...res.data, loadMore: payload.loadMore, previousList: payload.previousList }
      })
      .catch((error) => {
        return { data: [] };
      });
...
export const addtoFavorite = createAsyncThunk(
  ...
  async (payload = {}, reduxInfo) => {
    if (payload.success) {
      ...
      let res = await fetchClient
        .post(
          `${constants.API.COUNTRY.ADD_SINGLE_FAV_COUNTRY}${payload.id}`)
        .then((res) => {
          setTimeout(() => {
            reduxInfo.dispatch(
              clearUserSuccess(
...
export const getPublicHolidayList = createAsyncThunk(
  ...
  async (payload = {}) => {
    let res = await fetchClient
      .get(`${constants.API.COUNTRY.GET_HOLIDAY_LIST}${payload.id}/year/${payload.year}`,
    )
      .then(res => {
        return res.data
      })
  ...
  extraReducers: {
    ...
    [getAllCountriesList.fulfilled]: (state, action) => {
      if (action.payload?.data != null) {
        ...
        state.allCountriesList = action.payload?.loadMore ? [...action.payload.previousList, ...action.payload?.data] : action.payload?.data || [];
        state.allCountriesLoading = false;
      } else if(action.payload?.previousList && action.payload.previousList.length > 0) {
        state.allCountriesLength = 1;
        state.allCountriesList = action.payload?.loadMore ? [...action.payload.previousList] : action.payload?.data || [];
        state.allCountriesLoading = false;
      } else {
    ...
    [getPublicHolidays.fulfilled]: (state, action) => {
      let id = [];
      action.payload?.data?.holidays?.forEach((item) => {
        id.push({
    ...
    [employeeLifeCycle.fulfilled]: (state, action) => {
      ...
      action?.payload?.data.forEach((item) => {
        if (item.templateName === 'Employee Management - Employee Lifecycle - Attract') {
          attractId.push(item.solutionId);
        }
        if (item.templateName === 'Employee Management - Employee Lifecycle - Onboard') {
          onBoardId.push(item.solutionId);
        }
        if (item.templateName === 'Employee Management - Employee Lifecycle - Develop and Retain') {
          developId.push(item.solutionId);
        }
        if (item.templateName === 'Employee Management - Employee Lifecycle - Offboard') {
    ...
    [addtoFavorite.fulfilled]: (state, action) => {
      state.countryStatus.countryadded = action?.payload?.responseCode || undefined;
    ...
    [deleteFromFavorite.fulfilled]: (state, action) => {
      state.countryStatus.detetedSuccess = action?.payload?.responseCode || undefined;
    },
...
export const { clearUserSuccess } = country.actions
