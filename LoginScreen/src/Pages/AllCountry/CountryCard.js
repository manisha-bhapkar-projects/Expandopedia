...
export const CardLoader = (props) => (
    <div className="country-card" key={Math.random().toString(36).substr(2, 5)}>
...
export const CountryCard = ({ item, onNavigateCountryDetailsPage }) => {
    if(item.topics != null) {
        return (
            <div className="country-card" key={Math.random().toString(36).substr(2, 5)} onClick={() => onNavigateCountryDetailsPage(item)}>
                <div className="country-image-wrapper">
                    <h3>
                        {item.countryName}
                        {
