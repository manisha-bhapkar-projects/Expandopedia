...
import InsightsAndAnalysis from './InsightsAndAnalysis'
...
const setup = (store, historyMock) => {
    ...
    return mount(<Provider store={store}><Router history={historyMock}><InsightsAndAnalysis /></Router></Provider>);
}
...
describe(" Insight and Analysis page ", () => {
    ...
    test("Render the function when click on tags without error", () => {
        ...
        expect(tagClick.simulate('click').length).toBe(1)
    })
    ...
    test("Render function when click on textChange without error", () => {
        ...
        expect(textChange.simulate('change').length).toBe(1)
    })
