...
import { getArticlesCountryPage, employeeLifeCycle, getEmployeeLifeCycle, callgetUserCountryDetails } from "../../Store/reducers/country";
...
const Details = (props) => {
  ...
  useEffect(() => {
    ...
    if (id != null) {
      ...
      dispatch(employeeLifeCycle({
        "solutionName": ["Employee Management - Employee Lifecycle - Attract",
  ...
  useEffect(() => {
    if (ExpertSoultionId != null) {
      if (countryName) {
        callGetExpertProfiledataAPIAction_details(ExpertSoultionId, countryName.country_Name);
      }
    }
  }, [ExpertSoultionId])
  useEffect(() => {
    if (countryName) {
      callGetInsightsSolutionAction_details(insightsId, countryName.country_Name)
    }
  }, [insightsId])
  useEffect(() => {
  ...
  const callGetInsightsSolutionAction_details = (country_id, country) => {
    ...
    .catch((errors) => { });*/}
    if (country) {
      dispatch(getArticlesCountryPage({
  ...
  const callGetSnapShotDataAPIAction_details = (snapshot_id, countr_id) => {
    props
      .callGetSnapShotListDataAPIAction(snapshot_id, countr_id != null ? countr_id : id)
      .then((response) => {
  ...
  const callGetSpecificsAPIDataAction_details = (specificsId_data, id, _sTopicContentId) => {
    ...
    props
      ...
      .catch((errors) => { console.log(errors); setTopicsLoading(false) });
    ...
    if (!labourAndProductivity) {
      ...
      if (data && data.length > 0) {
        data.map((item) => {
          ...
          // console.log('isValidLabour_productivity', item)
          if (item.isPercent === "true") {
            if (item.maxValue > 0 && (item.value).toFixed(0) < 1) {
              setLabourAndProductivity(true);
            }
          } else {
            if ((item.value / defaultValue).toFixed(0) < 1 && item.maxValue > 0 && item.median > 0 && item.average > 0) {
              setLabourAndProductivity(true);
            }
          }
        });
    ...
    <React.Fragment>
      <div className="ip_header_wrap">
        <div className="ip_banner-img">
          <img src={countryName?.header_Image_Id ? `${constants.IMAGE.DOWNLOAD}${countryName.header_Image_Id}` : ""} />
        </div>
        <div className="ip_banner_gradient">
        </div>
        <div className="container-fluid">
          <div className="row">
            ...
            <div className="breadcrumb-w">
              <nav aria-label="breadcrumb">
                <ol className="breadcrumb">
                  {
                    country ?
                      <li className="breadcrumb-item"><Link to="/all-country">All Country</Link></li> :
            ...
            <div className="country-name-container">
              ...
              <img src={countryName?.flag_Upload_Id ? `${constants.IMAGE.DOWNLOAD}${countryName.flag_Upload_Id}` : ""}></img>
            </div>
              ...
              <div className="row">
                <div className="col-lg-9">
                  <div className="updated-info">
                    ...
                    <div className="">
                      <img alt='' src={userIcon} name="person-outline" className="user-icon" />
                      <span className="pl-3">{employeeCount ? employeeCount : "0"}</span><span> Employees</span>
                ...
                <div className="col-lg-3 col-12 d-flex justify-content-end">
                  <span className="actions" onClick={() => {
                    ...
                    <img alt="" src={compareIcon} name="git-compare-outline" className="compare" />
                    <span className='link-style'>Compare</span>
                  </span>
                  {popRender &&
                    <ComparePopup id={id} show={showDialog} onHide={() => {
                      setPopRender(false)
                      setShowDialog(false)
                    }} />}
                  <span className="actions" onClick={OpenSharePopupModal}>
                    <img alt="" src={shareIcon} name="share-outline" className="share" />
                    <span className='link-style'>Share</span>
                  </span>
                  <img alt="" src={pinIcon} href className="push-pin-icon" />
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="row country-content-mt">
        <Alertcard card_data={alertsData} title="Alerts" card_id="alerts" notification_color={true} icon="ph-bell" icon_name=""></Alertcard>
        <Alertcard card_data={countryArticles} title="Insights &amp; Analysis" card_id="insights" notification_color={false} icon="ph-file" icon_name="document-outline" countryName={countryName}></Alertcard>

        <Expertimage card_data={ExpertSoultionData}></Expertimage>
      </div>
      <div className="">
        <div className="tab-container">
          <ul className="col-md-9">
            <li><a href className={`cursor-pointer ${activeItem === 1 ? "active" : ""}`} onClick={() => setActiveItem(1)}>LABOR &amp; EMPLOYMENT</a></li>
            <li><a href className={`cursor-pointer ${activeItem === 2 ? "active" : ""}`} onClick={() => setActiveItem(2)} >EMPLOYEE LIFECYCLE</a></li>
            <li className="display_comingsoon"><a href className={`cursor-pointer ${activeItem === 3 ? "active" : ""}`} onClick={() => setActiveItem(3)}>COMPLIANCE</a> <ComingSoon direction="bottom"></ComingSoon> </li>
            <li><a href className={`cursor-pointer ${activeItem === 4 ? "active" : ""}`} onClick={() => setActiveItem(4)}>INDICATORS &amp; TRENDS</a></li>
          </ul>
          {activeItem === 1 &&
            <SnapShot
      ...
      {activeItem < 3 &&
        <>
          <div className="row scrollspy-header specific-header">
            <div className="col-md-3 d-flex align-items-center" id="specifics-header">
              <h3>The Specifics</h3> {topicsLoading && <Spinner className="specifics-loading-spinner" animation="border" size="sm" />}
            </div>
          ...
          {
            (!topicsLoading && specificsData && specificsData.length && (activeItem === 1 || (activeItem === 2))) &&
            <div className="scrollspy-content specific-data">
              <CountrySpecifics selectedSolution={selectedSolution} specifics={specificsData} divScroll={props.location.state} />
            </div>
          }
        </>
      }
