...
const CountryConfigurationList = (props) => {
  ...
  const loadCountryDetails = async (reload) => {
    ...
    if ((allCountryList.length === 0 || reload) && user_data && user_data?.userId) {
      ...
      console.log('CountryConfigurationList', props)
    }
    setLoading(false);
  };
  ...
  return (
    <div className="country-config-list-container loader-enable">
      {
        loading &&
      ...
      <div className="header-section">
        ...
        </h3>
        {
          addNewCountry.active ?
            (<>
              <span className="cancel-btn ml-auto" onClick={cancelAddNew}>Cancel</span>
              <span className="add-Country" onClick={() => saveCountryDetails("add_country")}>Add</span></>) :
            <>
              {
                onFormData ? (
                  <>
                    <span className="cancel-btn ml-auto" onClick={() => cancelEdit(false)}>Cancel</span>
                    <span className="add-Country" onClick={() => saveCountryDetails("update_country", countryDetail.id)}>Save</span>
                  </>
                ) :
                  <span onClick={() => { setSelectedCountry({}); setCountryDetail({}); setAddNewCountry({ active: true }) }} className="add-Country ml-auto">Add Country</span>
              }
            </>
        }
      </div>
            ...
            <div className="col-md-4 pr-0">
              <div className="coutry-list-container">
                <div className="search-country-wrapper">
                  <input type="text" value={searchText} onChange={(e) => onSearchCountry(e.target.value)} placeholder="Search by country or US state "></input>
                  {
                    searchText.length > 0 &&
                    <img className="search-icon" src={close} onClick={() => onSearchCountry("")} alt="" />
                  }
                </div>
