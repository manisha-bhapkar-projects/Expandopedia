          ...
          <div className="search-results">
            ...
            {/* Search result block .slice((pageNumber - 1) * pageSize, pageNumber * pageSize)*/}
            {
              searchResult.map((data, i) => {
                return (
                  <div key={i} className={data?.parentMetaDataInfo?.templateType === 'The Specifics' || data?.parentMetaDataInfo?.templateType === 'Article' || data?.parentMetaDataInfo?.templateType === 'Snapshot' ? "result-container pointer" : "result-container"} onClick={data?.parentMetaDataInfo?.templateType === 'The Specifics' ?
                    () => onNavigateCountry({
                    ...
                    <h4 className="regular">{(data?.parentMetaDataInfo?.templateType === 'The Specifics' || data?.parentMetaDataInfo?.templateType === 'Snapshot' || data?.parentMetaDataInfo?.templateType === 'Attract' || data?.parentMetaDataInfo?.templateType === 'Develop and Retain' || data?.parentMetaDataInfo?.templateType === 'Offboard' || data?.parentMetaDataInfo?.templateType === 'Onboard') && data.countryName && data.contentName.length > 1 ? data.countryName + " -" : " "} {data.alertTitle ? data.alertTitle : ''} {data.contentName} {data?.parentMetaDataInfo?.templateType === 'Snapshot' && data.countryName ? " -" + data?.parentMetaDataInfo?.templateType : ""}</h4>
